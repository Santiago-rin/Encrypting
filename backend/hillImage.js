...
current_folder_path = String(pathlib.Path(__file__).parent.resolve());
data_folder_path = current_folder_path + '/data';


function transform(np_array, shape) {
    return np_array.reshape(shape).astype('uint8');
}

function read_image(image_file_name) {
...
class Hill) {
    function __init__(self, data, file_name, key_path=null) {
...
function hillImageEncryption(image, image_file_name, hill, original_shape) {
    // -----------------------------------------------------------------
    // ------------------------- Encoding part -------------------------
    // -----------------------------------------------------------------
}
...
function hillImageDecryption(image, image_file_name, hill, original_shape) {
...
function finalHillImage(image_file_name) {
