...
function list_to_string(list_) {
    return [str(element) for (element in list_];
}

function list_to_int(list_) {
...
function decrypt(en_msg, p, key, q) {
    ...
    h = power(p, key, q);
    for (i in range(0, len(en_msg))) {
        dr_msg.push(chr(int(en_msg[i] / h)));
    }
    return dr_msg;
}
...
function ElGamalEncryption(plain_text) {
...
function ElGamalDecryption(plain_text, key_and_p) {
