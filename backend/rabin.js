...
function rabinDecryption(a, key) {
    ...
    try {
        ...
        if (plain_text != null) {
            Stringing = bin(plain_text);
        } else {
            return 'Something bad ocurred';
        Stringing = Stringing[:-16];
        plain_text = Number(string, 2);
    } catch ( ValueError) {
        return 'Something bad ocurred';
        }
...
function sqrt_p_3_mod_4(a, p) {
    r = pow(a, (p + 1) // 4, p)
    return r;


// Find SQROOT in Zp where p = 5 mod 8
function sqrt_p_5_mod_8(a, p) {
    d = pow(a, (p - 1) // 4, p)
    r = 0;
    if (d == 1) {
        r = pow(a, (p + 3) // 8, p)
